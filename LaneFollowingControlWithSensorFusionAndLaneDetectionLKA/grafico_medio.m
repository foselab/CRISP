...
grid on;
filename = fullfile('C:\Users\Luca\Desktop\tesi LKA\grafici\OTA3','grafico_random_medio_CONF3.fig');
savefig(filename);
...
vettore_failure_binario_APDISC = zeros(1,length(vettore_failure_APDISC));
...
figure; 
p=plot(asse_x, vettore_failure_binario,'g-o', asse_x, vettore_failure_medio,'r--s', asse_x,vettore_failure_binario_APDISC,'b:d');
p(1).LineWidth=2;
p(2).LineWidth=2;
p(3).LineWidth=2;
...
filename = fullfile('C:\Users\Luca\Desktop\tesi LKA\grafici\OTA3','grafico_CONFRONTO_CONF3.fig');
savefig(filename);
